...
internal void Win32InitWASAPI(int32 SamplesPerSecond, int32 BufferSizeInSamples)
{
...
internal void Win32FillSoundBuffer(win32_sound_output *SoundOutput, int SamplesToWrite, game_sound_buffer *SourceBuffer)
{
    BYTE* SoundBufferData;
...
internal void Win32GetKeyState(game_keyboard_input *Input)
{
...
inline LARGE_INTEGER Win32GetWallClockTime()
{
...
internal FILETIME Win32FileLastAccessTime(char *filename)
{
...
internal win32_game_code LoadHandmadeDynamic(char *SourceFilename)
{
...
internal void UnloadHandmadeDynamic(win32_game_code *Code)
{
    if (Code->Library) {
        ...
        if(Window) {
            ...
            if (Samples && Memory.PermenantStorage && Memory.TempStorage) {
                while(GlobalRunning) {
                    ...
                    // FIXME: Remove this
                    Input.Q.WasDown = false;
                    Input.Q.IsDown = false;
                    Input.E.WasDown = false;
                    Input.E.IsDown = false;
                    ...
                    while(PeekMessage(&Message, 0, 0, 0, PM_REMOVE)) {
                        switch(Message.message) {
                            ...
                            case WM_KEYDOWN: 
                                {
                                    uint32 VKCode = (WORD) Message.wParam;
