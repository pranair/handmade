	...
	*/
	...
	if(RegisterClass(&WindowClass)) {
		...
		if(Window) {
			...
			// NOTE: This finds the minimum latency to play the data written into the buffer
			REFERENCE_TIME MinimumDevicePeriod;
			REFERENCE_TIME DefaultDevicePeriod;
			GlobalSoundClient->GetDevicePeriod(&DefaultDevicePeriod, &MinimumDevicePeriod);
			// NOTE: REFERENCE_TIME is in 100-nanosecond units
			//SoundOutput.LatencySampleCount = (uint32) (SoundOutput.SamplesPerSecond * (MinimumDevicePeriod / (1000.0 * 1000.0))); 
			...
			if (Samples && Memory.PermenantStorage && Memory.TempStorage) {
				while(GlobalRunning) {
					...
					if (SoundPaddingSize > 0) {
						char OutputBuffer[256];
						sprintf_s(OutputBuffer, 256, "Padding: %.2f\n", (float)(BufferSizeInFrames/SoundPaddingSize));
					...
					if (FrameSecondsElasped < TargetSecondsPerFrame) {
						if (SleepIsGranular) {
							...
							//       Does this matter?
							DWORD SleepMS = (DWORD) (1000.0f * (TargetSecondsPerFrame - FrameSecondsElasped) - 1);
							if (SleepMS > 0) {
								Sleep(SleepMS);
							}
					...
					//char OutputBuffer[256];
					//sprintf_s(OutputBuffer, 256, "ms/f: %.2f\n", MSPerFrame);
					//OutputDebugStringA(OutputBuffer);
