...
void GameFillSoundBuffer(game_sound_buffer *SoundBuffer, game_state *State)
{
...
GAME_UPDATE_RENDER(GameUpdateAndRender)
{
    ...
    if (KBInput->Down.WasDown) {
        PlayerY += 20;
        //PlayerY += 20;
        State->YOffset -= 20;
    }

    if (KBInput->Up.WasDown) {
        PlayerY -= 20;
        //PlayerY -= 20;
        State->YOffset += 20;
    }

    if (KBInput->Right.WasDown) {
        PlayerX += 20;
    }

    if (KBInput->Left.WasDown) {
        PlayerX -= 20;
    }
